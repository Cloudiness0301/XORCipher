...
namespace XORCipher
{
    public class XORCipher
    {
        ...
        private string EncryptDecrypt(string text, int key)
        {
            ...
            {
                result += ((char)(text[i] ^ secretKey[i])).ToString();
            }
            return result;
        }
        ...
        public string DecryptRandomKey(string encryptedText, int key)
            => EncryptDecrypt(encryptedText, key);
        ...
        private string EncryptDecrypt(string text, string password)
        {
            ...
            for (var i = 0; i < text.Length; i++)
            {
                result += ((char)(text[i] ^ secretPassword[i])).ToString();
            }
            return result;
        }
        ...
        public string Encrypt(string plainText, string password)
            => EncryptDecrypt(plainText, password);

        //text decryption
        public string Decrypt(string encryptedText, string password)
            => EncryptDecrypt(encryptedText, password);
    }
